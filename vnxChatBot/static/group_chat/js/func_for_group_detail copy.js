...
// 1. Khi người dùng bấm nút Reply trên thanh công cụ nổi của tin nhắn
function prepareReplyMessage(messageId, senderName, messageContent) {
    currentReplyToId = messageId;

    // Hiện thanh preview trích dẫn
...
// 3. Khi gửi tin nhắn qua WebSocket, đính kèm thêm reply_to_id
/**
 * File: apps/group_chat/static/group_chat/js/func_for_group_detail.js
 * Mục đích: Xử lý toàn bộ logic tương tác real-time, gửi tin nhắn, reply và feedback kiểu Zalo cho workspace nhóm.
 */
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    // Khởi tạo biến toàn cục chatSocket nếu chưa có
    if (typeof window.chatSocket === 'undefined' || !window.chatSocket) {
        window.chatSocket = new WebSocket(
            wsScheme + window.location.host + '/ws/chat/' + groupId + '/'
        );
    }

    // 2. Xử lý sự kiện nhận tin nhắn từ WebSocket Server
    window.chatSocket.onmessage = function (e) {
        const data = JSON.parse(e.data); // Định nghĩa biến data an toàn trong phạm vi hàm nhận
    ...
    window.chatSocket.onclose = function (e) {
        console.error('WebSocket chat socket đóng bất ngờ.');
    };

    // 3. Xử lý sự kiện gửi tin nhắn và quản lý trạng thái Reply
    ...
    if (chatForm && chatInput) {
        chatForm.onsubmit = function (e) {
            ...
            if (window.chatSocket && window.chatSocket.readyState === WebSocket.OPEN) {
                ...
                console.error("WebSocket chưa sẵn sàng kết nối.");
            }
        };
    }
