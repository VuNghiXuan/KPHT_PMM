...
class KnowledgeDraft(models.Model):
    ...
    project = models.ForeignKey(
        'app_miner.DataSource', # Cập nhật tên app label mới nếu cần
    ...
    def save(self, *args, **kwargs):
        ...
        # 2. AUTO-SYNC: Cập nhật mô tả vào DataEntry để đồng bộ hệ thống Miner
        if self.sheet and self.content:
            try:
                # Đảm bảo đường dẫn import đúng với cấu trúc folder apps/
                from apps.app_miner.models import DataEntry 
                DataEntry.objects.filter(id=self.sheet.id).update(description=self.content)
            except ImportError:
                logger.error("Không thể import DataEntry để đồng bộ.")
